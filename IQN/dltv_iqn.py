...
class QNetwork(nn.Module):
    ...
    def forward(self, x, q_stats=False):
        ...
        left_truncated = int(N_QUANT / 2)
        #calc variance
        q_var, q_mean = torch.var_mean(action_value[:, :, -left_truncated:], dim=2)
        # q_var, q_mean = torch.mean(q_var.detach().cpu(),dim=1), torch.mean(q_mean.detach().cpu(),dim=1) #(bs, N_ACTIONS)
...
#
################################################################################################################
def world_dynamics(t, replay_start_size, num_actions, s, env, policy_net, constant):

    # A uniform random policy is run before the learning starts
    if t < replay_start_size and t>=0:
        action = torch.tensor([[random.randrange(num_actions)]], device=device)

    elif t == -1:
        if numpy.random.binomial(1, END_EPSILON/10) == 1:
            action = torch.tensor([[random.randrange(num_actions)]], device=device)
        else:
            ...
            with torch.no_grad():
                ...
                action = policy_net(s)[0].mean(dim=2).argmax(dim=1).view(1,1)
    else:
        ...
        if numpy.random.binomial(1, epsilon) == 1:
            ...
            with torch.no_grad():
                # action = policy_net(s).max(1)[1].view(1, 1)
                q_var = policy_net(s)[2] #(bs, N_ACTIONS)
                # print('qvar:', q_var)
                action = policy_net(s)[0].mean(dim=2) #(bs, N_ACTIONS)
                # print('action', action)
                action = torch.argmax((action + constant * torch.sqrt(torch.log(torch.tensor(t) / t)) * q_var), dim=1).view(1, 1)
...
def eval_episode(test_env, num_actions, policy_net, constant):
    G = 0.0

    # Initialize the environment and start state
    test_env.reset()
    s = get_state(test_env.state())
    is_terminated = False
    while(not is_terminated):
        # Generate data
        s_prime, action, reward, is_terminated = world_dynamics(-1, 0, num_actions, s, test_env, policy_net, constant)
        G += reward.item()
...
def dqn(env, test_env,replay_off, target_off, output_file_name, constant=50, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE, wandb=None, rs=None):
...
def main():
    ...
    parser.add_argument('--constant', type=float, default=50, help='constant for bonus decaying')
    ...
    test_env = Environment(args.game)


    print('Cuda available?: ' + str(torch.cuda.is_available()))

    wandb.init(config=args,project="MiniAtari", entity="risk_scheduling", group=args.game, name=args.scheduling+'_dltv_'+str(args.constant))
    ...
    dqn(env, test_env, args.replayoff, args.targetoff, file_name, args.constant, args.save, load_file_path, args.alpha, wandb, rs)

...
def eval_episode(test_env, num_actions, policy_net):
    G = 0.0
    ...
    while(not is_terminated):
        # Generate data
        s_prime, action, reward, is_terminated = world_dynamics(-1, 0, num_actions, s, test_env, policy_net)
        G += reward.item()
...
def dqn(env, test_env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE, wandb=None, rs=None):
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if t % EVAL_FREQ == 0:
                ...
                for _ in range(EVAL_NUM):
                    eval_G += eval_episode(test_env, num_actions, policy_net)
                wandb.log({
...
def main():
    ...
    dqn(env, test_env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha, wandb, rs)
